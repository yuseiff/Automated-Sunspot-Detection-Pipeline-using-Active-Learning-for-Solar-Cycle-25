...
SLICE_WH = (640, 640)
# Reverted 0.35 -> 0.2: the overlap experiment didn't help. mAP@50-95 actually
# went slightly down (0.181 -> 0.173) and precision/F1 dropped too, on a
# near-identical image subset. The mAP@50 vs mAP@50-95 gap looks like a
# model box-regression quality issue rather than something slicing overlap
# can fix, so no reason to pay the extra runtime for more overlap.
OVERLAP_RATIO_WH = (0.2, 0.2)

# --- Confidence thresholds: deliberately split into two roles -------------
# OP_CONF_THRESHOLD is the real deployment/operating threshold: used for the
# single-image visualization, and for precision/recall/F1 during evaluation
# (those metrics are only meaningful at one fixed confidence cutoff).
# Found via experiment: 0.25 gave precision=0.23/recall=0.72/F1=0.35;
# 0.5 gave precision=0.45/recall=0.62/F1=0.52 -- 0.5 is the current winner.
OP_CONF_THRESHOLD = 0.5

# MAP_CONF_THRESHOLD is kept very low and used ONLY to generate the detection
# set that feeds mAP@50 / mAP@50-95 during evaluation. mAP is supposed to
# integrate precision/recall across the *entire* confidence range, so
# filtering to OP_CONF_THRESHOLD before computing it would truncate that
# curve and artificially depress the score. Detections below OP_CONF_THRESHOLD
# are still excluded from precision/recall/F1 by filtering afterward.
# Raised 0.001 -> 0.02: at 0.001, a single 4096x4096 image produced 7,267 raw
# candidate boxes, and the stitching step's NMS needs an all-pairs IoU matrix
# that scales quadratically with box count -- that's what triggered a
# MemoryError. 0.02 is still low enough to give mAP a full curve to
# integrate over, but keeps the raw candidate count in a safer range,
# especially now that SLICE_WH=640 roughly triples the tile count per image.
MAP_CONF_THRESHOLD = 0.02

# Cross-tile stitching threshold: how much two overlapping-tile detections
# need to overlap before InferenceSlicer merges them into one box.
# Next experiment: lowering 0.5 -> 0.3 to test whether more aggressive
# merging reduces duplicate detections of the same real spot at tile seams.
IOU_THRESHOLD = 0.3
...
def load_yolo26(model_path: str = YOLO_MODEL_PATH, conf_threshold: float = OP_CONF_THRESHOLD):
    ...
    `conf_threshold` is bound into the returned predict_fn -- pass
    OP_CONF_THRESHOLD for normal/deployment use, or MAP_CONF_THRESHOLD when
    you need the full, unfiltered detection set (e.g. for mAP).
...
def load_RF_DETR(model_path: str = RFDETR_MODEL_PATH, conf_threshold: float = OP_CONF_THRESHOLD):
    """
...
def run_workflow(model: str = MODEL_TYPE) -> dict:
    ...
    # Uses OP_CONF_THRESHOLD -- this is a real deployment-style output.
    _, predict_fn, class_names = load_model(model, conf_threshold=OP_CONF_THRESHOLD)
...
# compute mAP@50, precision, recall, and F1 by comparing stitched predictions
# against ground-truth annotations.
#
# Confidence handling: inference runs ONCE per image at MAP_CONF_THRESHOLD
# (very low), giving the full detection set. mAP is computed on that full
# set. Precision/Recall/F1 are computed on the subset filtered down to
# OP_CONF_THRESHOLD -- the actual deployment operating point.
#
# Logging:
...
def evaluate_workflow(model: str = MODEL_TYPE) -> dict:
    ...
    # Single low-threshold model load -- gives the full detection set that
    # both mAP and (after filtering) precision/recall/F1 are computed from.
    _, predict_fn, _ = load_model(model, conf_threshold=MAP_CONF_THRESHOLD)
    slicer = build_slicer(predict_fn)
    ...
    run_config = {
        ...
        "map_conf_threshold": MAP_CONF_THRESHOLD,
        "iou_threshold": IOU_THRESHOLD,
        "thread_workers": THREAD_WORKERS,
    ...
    fieldnames = [
        ...
        "image_path",
        "elapsed_sec",
        "num_predictions_map",   # full detection count (MAP_CONF_THRESHOLD)
        "num_predictions_op",    # filtered to OP_CONF_THRESHOLD
        "num_ground_truth",
        "mAP@50",
        "mAP@50-95",
        "precision@50",
    ...
    try:
        for idx, (image_path, image, ground_truth) in enumerate(progress_bar, start=1):
            predictions_full: sv.Detections = slicer(image)
            predictions_op = predictions_full[predictions_full.confidence >= OP_CONF_THRESHOLD]

            # mAP integrates precision/recall across the full confidence
            # range, so it gets the unfiltered detection set.
            map_metric.update(predictions_full, ground_truth)

            # Precision/Recall/F1 are meaningful at one fixed confidence
            # cutoff -- the actual deployment operating point.
            precision_metric.update(predictions_op, ground_truth)
            recall_metric.update(predictions_op, ground_truth)
            f1_metric.update(predictions_op, ground_truth)
            ...
            writer.writerow({
                ...
                "num_predictions_map": len(predictions_full),
                "num_predictions_op": len(predictions_op),
            ...
            progress_bar.set_postfix(
                pred=len(predictions_op),
                gt=len(ground_truth),
                mAP50=metrics["mAP@50"],
                P=metrics["precision@50"],
                R=metrics["recall@50"],
            )
